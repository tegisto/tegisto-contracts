...
async function main() {
  ...
  const idoContract: SimpleVesting = await ethers.getContract('SimpleVestingAltava2', deployer);
  const id = await idoContract.idoId();
  console.log(id);

  return new Promise<void>((resolve, reject) => {
    const inputStream = fs.createReadStream('altava_round_2_2.csv', 'utf8');
...
async function generateSignatures(idoId: string, accounts: AccountData[], signer: SignerWithAddress) {
  ...
  fs.writeFileSync('altavalist2_2.json', JSON.stringify(wihiteList));
}
