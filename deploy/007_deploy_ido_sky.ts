...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if ([31337, 44787].includes(chainId)) {
    skyAddress = (await hre.deployments.get('SkynetToken')).address;
  ...
  if (chainId === 31337) {
    ...
    whitelistEndDate = startDate;
    endDate = startDate + 60 * 60 * 34;
  }

  if (startDate === 0 || skyAddress === '' || tgsAddress === '') {
    throw new Error(`Unsupported chainId: ${chainId}`);
  }

  const deployResult: DeployResult = await deploy('SkynetSale', {
    contract: 'TokenPublicSale',
    from: deployer,
    args: [
      skyAddress, // address _token,
      ...
      hre.ethers.utils.solidityKeccak256(['string'], ['SkynetSale']),
    ],
    log: true,
    autoMine: true, // speed up deployment on local network (ganache, hardhat), no effect on live networks
  });

  const contract: TokenPublicSale = await hre.ethers.getContract('SkynetSale', deployer);
...
export default func;
func.id = 'deploy_ido_sky'; // id required to prevent reexecution
func.tags = ['SkynetSale'];
